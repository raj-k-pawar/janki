...
class BookingModel {
  ...
  String city;
  String mobile;
  String batchType; // full_day, morning, afternoon
  int guestsAbove10;
  ...
  // Generate QR data string for canteen
...
ID: ${id ?? 'NEW'}''';
  ...
  // Calculate food deductions
  double getFoodDeduction() {
    double deduction = 0;
    if (!foodBreakfast && (batchType == 'full_day' || batchType == 'morning')) {
      deduction += 50 * totalGuests;
    }
    if (!foodLunch && (batchType == 'full_day' || batchType == 'morning')) {
      deduction += 100 * totalGuests;
    }
    if (!foodHighTea && (batchType == 'full_day' || batchType == 'afternoon')) {
      deduction += 50 * totalGuests;
    }
    if (!foodDinner && batchType == 'afternoon') {
      deduction += 100 * totalGuests;
    }
    return deduction;
  }
...
class WorkerModel {
  ...
  Map<String, dynamic> toJson() => {
    'name': name, 'role': role, 'mobile': mobile,
    'salary': salary, 'joining_date': joiningDate, 'status': status,
  };
}
