...
class _AddCustomerScreenState extends State<AddCustomerScreen> {
  ...
  int    get _adults   => int.tryParse(_adults10.text) ?? 0;
  int    get _children => int.tryParse(_child3.text) ?? 0;
  ...
  double get _aAmt     => _adults * _aRate;
  double get _cAmt     => _children * _cRate;
  double get _advance  => double.tryParse(_advCtrl.text) ?? 0;

  double get _foodDeduction {
    if (_pkg == null) return 0;
    double d = 0;
    if (_pkg!.breakfast) d += (_total - (int.tryParse(_breakCtrl.text) ?? 0)).clamp(0, 999) * 50;
    if (_pkg!.lunch)     d += (_total - (int.tryParse(_lunchCtrl.text) ?? 0)).clamp(0, 999) * 100;
    if (_pkg!.snacks)    d += (_total - (int.tryParse(_snackCtrl.text) ?? 0)).clamp(0, 999) * 50;
    if (_pkg!.dinner)    d += (_total - (int.tryParse(_dinnerCtrl.text) ?? 0)).clamp(0, 999) * 100;
    return d;
  }

  double get _totalAmt => (_aAmt + _cAmt - _foodDeduction - _advance).clamp(0, double.infinity);

  @override
  void initState() {
    super.initState();
    _loadPackages();
    for (final c in [_adults10, _adultRate, _child3, _childRate,
  ...
  void _syncFood() {
    if (_pkg == null) return;
    setState(() {
      if (_pkg!.breakfast) _breakCtrl.text = _total.toString();
      if (_pkg!.lunch)     _lunchCtrl.text = _total.toString();
      if (_pkg!.snacks)    _snackCtrl.text = _total.toString();
      if (_pkg!.dinner)    _dinnerCtrl.text= _total.toString();
    });
  }

  @override
  void dispose() {
    for (final c in [_nameCtrl, _cityCtrl, _phoneCtrl, _adults10, _adultRate,
        _child3, _childRate, _breakCtrl, _lunchCtrl, _snackCtrl, _dinnerCtrl, _advCtrl]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    if (_pkg == null) { showSnack(context, 'Please select a package', error: true); return; }
    setState(() => _loading = true);

    final isEdit = widget.existing != null;
    final id = isEdit ? widget.existing!.id : DateTime.now().millisecondsSinceEpoch.toString();
    ...
    await StorageService.instance.saveCustomer(c);
    ...
    if (isEdit) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => QrConfirmScreen(customer: c)),
      );
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              if (_packages.isEmpty)
                ...
                ..._packages.map((pkg) => _pkgCard(pkg)),
              ...
              if (_pkg != null) ...[
                ...
                WhiteCard(child: Column(children: [
                  ...
                  Row(children: [
                    Expanded(child: _nf(_adults10, 'No. of Adults',
                        onChange: (_) => _syncFood())),
                    const SizedBox(width: 10),
                    Expanded(child: _nf(_adultRate, 'Rate per person')),
                  ]),
                  const SizedBox(height: 6),
                  _amtRow('Adults Amount', _aAmt),
                  const Divider(height: 20),
                  _subHead('👶 Children (3–10 yrs)'),
                  const SizedBox(height: 8),
                  Row(children: [
                    Expanded(child: _nf(_child3, 'No. of Children',
                        onChange: (_) => _syncFood())),
                    const SizedBox(width: 10),
                    Expanded(child: _nf(_childRate, 'Rate per person')),
                  ]),
                  const SizedBox(height: 6),
                  _amtRow('Children Amount', _cAmt),
                ])),
                ...
                WhiteCard(child: Column(children: [
                  Container(
                    ...
                    child: Text(
                      'Reduce guests below total to deduct:\nBreakfast/Snacks -50/guest, Lunch/Dinner -100/guest',
                      style: GoogleFonts.poppins(fontSize: 11, color: AppColors.warning),
                    ),
                  ),
                  const SizedBox(height: 10),
                  if (_pkg!.breakfast) _foodRow('Breakfast', _breakCtrl, '-50/g'),
                  if (_pkg!.lunch)     _foodRow('Lunch',     _lunchCtrl,  '-100/g'),
                  if (_pkg!.snacks)    _foodRow('Snacks',    _snackCtrl,  '-50/g'),
                  if (_pkg!.dinner)    _foodRow('Dinner',    _dinnerCtrl, '-100/g'),
                  if (_foodDeduction > 0) ...[
                    const Divider(height: 16),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Text('Food Deduction',
                          style: GoogleFonts.poppins(fontSize: 12, color: AppColors.error)),
                      Text('-${_foodDeduction.toStringAsFixed(0)}',
                ...
                WhiteCard(child: _nf(_advCtrl, 'Advance Amount')),
  ...
  Widget _pkgCard(PackageModel pkg) {
    final sel = _pkg?.id == pkg.id;
    return GestureDetector(
      onTap: () => _selectPkg(pkg),
      child: AnimatedContainer(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 5, children: [
            if (pkg.breakfast) _fTag('Breakfast', sel),
            if (pkg.lunch)     _fTag('Lunch', sel),
            if (pkg.snacks)    _fTag('Snacks', sel),
            if (pkg.dinner)    _fTag('Dinner', sel),
          ]),
          const SizedBox(height: 6),
          Row(children: [
            _pTag('Child: ${pkg.childPrice.toStringAsFixed(0)}', sel),
            const SizedBox(width: 8),
            _pTag('Adult: ${pkg.adultPrice.toStringAsFixed(0)}', sel),
          ]),
        ]),
      ),
    );
  ...
  Widget _fTag(String t, bool sel) => Container(
  ...
  Widget _pTag(String t, bool sel) => Container(
  ...
  Widget _summaryCard() => Container(
    ...
    child: Column(children: [
      ...
      _sRow('Adults Amt',   '${_aAmt.toStringAsFixed(0)}'),
      _sRow('Children Amt', '${_cAmt.toStringAsFixed(0)}'),
  ...
  Widget _amtRow(String l, double amt) => Container(
            ...
            WhiteCard(
              child: Column(
                ...
                children: [
                  ...
                  _row('Total Amount',  'Rs.${customer.totalAmount.toStringAsFixed(0)}', bold: true),
                  _row('Payment',
                      customer.paymentMode == PaymentMode.cash ? 'Cash' : 'Online',
                      bold: true),
                ],
              ),
            ),
