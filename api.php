...
if ($pdo->query("SELECT COUNT(*) FROM users")->fetchColumn() == 0) {
    ...
    $stmt->execute(['admin',    password_hash('admin123',    PASSWORD_DEFAULT), 'admin',   'Admin User']);
    $stmt->execute(['manager1', password_hash('manager123',  PASSWORD_DEFAULT), 'manager', 'Manager One']);
    $stmt->execute(['owner1',   password_hash('owner123',    PASSWORD_DEFAULT), 'owner',   'Owner']);
    $stmt->execute(['canteen1', password_hash('canteen123',  PASSWORD_DEFAULT), 'canteen', 'Canteen Staff']);
}
...
switch ($action) {
    ...
    case 'addBooking':
        $stmt = $pdo->prepare("INSERT INTO bookings
            (customer_name,city,mobile,batch_type,guests_above_10,amount_above_10,
             ...
             food_dinner,total_guests,total_amount,payment_mode,qr_code,booking_date)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->execute([
            $input['customer_name'], $input['city'], $input['mobile'], $input['batch_type'],
            $input['guests_above_10'], $input['amount_above_10'],
            ...
            $input['qr_code'] ?? null, $input['booking_date'] ?? date('Y-m-d')
        ]);
    ...
    case 'updateBooking':
        $stmt = $pdo->prepare("UPDATE bookings SET
            customer_name=?,city=?,mobile=?,batch_type=?,guests_above_10=?,amount_above_10=?,
            guests_3_to_10=?,amount_3_to_10=?,food_breakfast=?,food_lunch=?,food_high_tea=?,
            food_dinner=?,total_guests=?,total_amount=?,payment_mode=? WHERE id=?");
        $stmt->execute([
            $input['customer_name'], $input['city'], $input['mobile'], $input['batch_type'],
            $input['guests_above_10'], $input['amount_above_10'],
            $input['guests_3_to_10'], $input['amount_3_to_10'],
            $input['food_breakfast'] ? 1:0, $input['food_lunch'] ? 1:0,
            $input['food_high_tea'] ? 1:0, $input['food_dinner'] ? 1:0,
            $input['total_guests'], $input['total_amount'], $input['payment_mode'], $input['id']
        ]);
    ...
    case 'addWorker':
        ...
        $stmt->execute([$input['name'],$input['role'],$input['mobile'],$input['salary'],
            $input['joining_date'] ?? date('Y-m-d'), $input['status'] ?? 'active']);
        echo json_encode(['success' => true, 'id' => $pdo->lastInsertId()]);
        break;
